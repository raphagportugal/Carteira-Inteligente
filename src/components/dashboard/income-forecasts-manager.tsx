...
export function IncomeForecastsManager({ forecasts }: { forecasts: IncomeForecast[] }) {
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      ...
      setOpen(false); setError(""); showSuccess("Receita futura salva."); router.refresh();
    });
  }
  ...
  return (
    <section className="dashboard-card mt-6 p-6">
      <div className="flex items-center justify-between"><div><h2 className="text-lg font-extrabold">Receitas futuras</h2><p className="mt-1 text-xs text-slate-400">Valores esperados que complementam receitas realizadas.</p></div><button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white"><Plus className="size-4" /> Adicionar</button></div>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{forecasts.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl bg-slate-50 p-4"><div><p className="text-xs capitalize text-slate-400">{monthFormatter.format(parseDate(item.month))}</p><p className="mt-1 font-extrabold text-emerald-600">{formatCurrency(Number(item.expected_income))}</p></div><button onClick={() => remove(item.id)} className="text-slate-400 hover:text-red-500"><Trash2 className="size-4" /></button></div>)}{forecasts.length === 0 && <p className="text-sm text-slate-500">Nenhuma receita futura informada.</p>}</div>
      {open && <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/50 px-4"><button className="absolute inset-0" onClick={() => setOpen(false)} /><div className="relative w-full max-w-md rounded-3xl bg-white p-8"><button onClick={() => setOpen(false)} className="absolute right-5 top-5"><X className="size-5" /></button><h2 className="text-2xl font-extrabold">Receita futura</h2><form action={submit} className="mt-6 space-y-4"><label className="block"><span className="mb-2 block text-sm font-semibold">Mês</span><input name="month" required type="month" className="h-12 w-full rounded-xl border px-4" /></label><label className="block"><span className="mb-2 block text-sm font-semibold">Valor esperado</span><input name="expected_income" required type="number" min="0.01" step="0.01" className="h-12 w-full rounded-xl border px-4" /></label>{error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<button disabled={pending} className="h-12 w-full rounded-xl bg-slate-900 font-bold text-white">Salvar previsão</button></form></div></div>}
    </section>
  );
}
