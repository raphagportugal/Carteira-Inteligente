...
export function IncomeForecastsManager({ forecasts }: { forecasts: IncomeForecast[] }) {
  ...
  return (
    <section className="dashboard-card mt-6 p-6">
      <div className="flex items-center justify-between"><div><h2 className="text-lg font-extrabold">Entradas futuras</h2><p className="mt-1 text-xs text-slate-400">Valores esperados que complementam entradas realizadas.</p></div><button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white"><Plus className="size-4" /> Adicionar</button></div>
