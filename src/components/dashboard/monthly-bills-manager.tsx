...
import type { MonthlyBill } from "@/lib/finance/types";
import { showSuccess } from "@/lib/ui/feedback";

export function MonthlyBillsManager({ bills }: { bills: MonthlyBill[] }) {
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      const result = editing
        ? await updateMonthlyBill(formData)
        : await createMonthlyBill(formData);
      if (!result.success) {
    ...
    <>
      ...
      {bills.length === 0 ? (
        <EmptyState
          ...
          description="Cadastre despesas recorrentes para incluí-las nas projeções e despesas comprometidas."
          action={<button onClick={() => show()} className="focus-ring rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white">Cadastrar mensalidade</button>}
        />
      ) : (
        <>
          ...
          <section className="space-y-4">
            {bills.map((bill) => {
              const effectivelyActive =
                ...
                (!bill.end_date || bill.end_date >= today);
              return (
              <article key={bill.id} className="dashboard-card flex flex-col gap-5 p-5 md:flex-row md:items-center">
                ...
                <div className="min-w-0 flex-1"><h2 className="font-extrabold">{bill.name}</h2><p className="mt-1 text-xs text-slate-400">{bill.category} · {bill.payment_method}</p></div>
                <div className="grid grid-cols-2 gap-5 text-sm sm:grid-cols-4">
                  ...
                  <div><p className="text-[10px] uppercase text-slate-400">Status</p><span className={`mt-1 inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ${effectivelyActive ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}>{effectivelyActive ? "Ativa" : bill.end_date && bill.end_date < today ? "Encerrada" : "Inativa"}</span></div>
                </div>
