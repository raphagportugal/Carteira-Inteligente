...
function MonthDetails({ month }: { month: CashFlowMonth }) {
  const groups = [
    { label: "Movimentacoes", value: month.transactionExpenses, color: "bg-slate-700" },
  ...
  return (
    <details className="group mt-5 rounded-2xl border border-slate-100 bg-white/70">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-extrabold text-slate-900 transition hover:text-moss-700">
        <span>Ver mais</span>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold text-slate-500 group-open:bg-moss-50 group-open:text-moss-700">Perfil do mes</span>
      </summary>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
          ...
          <div className="rounded-2xl bg-slate-950 p-4 text-white">
            ...
            <div className="mt-5 space-y-3 text-xs">
              <RatioLine label="Taxa de poupanca" value={savingsRate} tone={metricTone(month.leftover)} />
...
type MonthScore = "Excelente" | "Bom" | "Atencao" | "Critico";
...
function monthScore(leftover: number, entries: number): MonthScore {
  if (leftover < 0) return "Critico";
  if (entries <= 0) return leftover > 0 ? "Bom" : "Atencao";
...
function monthInsight(month: CashFlowMonth, groups: Array<{ label: string; value: number }>) {
  if (month.leftover < 0) return "Atencao: as saidas superaram as entradas.";
  if (month.entries > 0 && month.leftover / month.entries >= 0.25) return "Boa capacidade de poupanca neste mes.";
  ...
  return `${biggest.label} teve o maior peso dos gastos, com ${formatPercent(biggest.value / month.expenses)} das saidas.`;
}
